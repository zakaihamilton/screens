...
screens.widget.window = function WidgetWindow(me, packages) {
    const { core } = packages;
    me.init = function () {
        me.element = {
            dependencies: {
    ...
    me.close = {
        ...
        set: function (object, value) {
            ...
            if (parent_window) {
                ...
                core.property.set(me.ui.element.workspace(), "widget.window.refocus");
            }
    ...
    me.exportMenuList = function (object, method) {
        ...
        items.push({
            text: "Clipboard",
            select: () => {
                core.property.set(window, method, me.ui.clipboard);
            }
        });
        if (!items.length) {
            return null;
