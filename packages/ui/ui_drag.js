...
package.ui.drag.icon = function UIDragIcon(me) {
    ...
    me.start = {
        set: function (object, event) {
            ...
            if (target.drag_element) {
                var rect = me.package.ui.rect.absolute_region(target.drag_element);
                var in_rect = me.package.ui.rect.in_region(rect, event.clientX, event.clientY);
                if (!in_rect) {
                    if (event.preventDefault) {
                        event.preventDefault();
                    }
            ...
            me.source = target;
            var source_rect = me.package.ui.rect.absolute_region(target);
    ...
    me.enter = {
        set: function (object, event) {
            if (me.source && me.source.style.position !== "absolute") {
                var target = me.parent_draggable(event.target);
                me.target = target;
                me.package.core.property.set(target, "ui.class.add", "over");
            }
        }
    };
    me.over = {
    ...
    me.leave = {
        set: function (object, event) {
            if (me.source && me.source.style.position !== "absolute") {
                var target = me.parent_draggable(event.target);
                me.package.core.property.set(target, "ui.class.remove", "over");
                me.target = null;
            }
        }
    };
        ...
        set: function (object, event) {
            if (me.source && me.source.style.position !== "absolute") {
                ...
                if (me.source.style.position !== "absolute") {
                    me.package.ui.node.shift(me.source, target);
                }
            }
            return false;
        }
    ...
    me.end = {
        set: function (object, event) {
            if (me.source) {
                if (me.source.style.position !== "absolute") {
                    var target = me.parent_draggable(event.target);
                    me.package.core.property.set(me.target, "ui.class.remove", "over");
                    me.package.core.property.set(target, "ui.class.remove", "over");
                }
    ...
    me.drag = {
        set: function (object, event) {
            if (me.source && me.source.style.position === "absolute") {
                ...
                var region = me.package.ui.rect.relative_region(me.source);
                region.left = me.drag_pos.x - me.drag_offset.x;
                region.top = me.drag_pos.y - me.drag_offset.y;
                me.package.ui.rect.set_relative_region(me.source, region);
            }
        }
    };
