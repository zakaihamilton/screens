...
function WidgetScrollbarTemplate(me, scroll_type) {
    ...
    me.container = function(object) {
        var container = null;
        if(object.component === me.the.widget.window.id) {
            container = object.var.container;
        }
        else {
            container = me.the.ui.node.container(object, me.the.widget.container.id);
        }
        return container;
    };
    me.alwaysShow = {
    ...
    me.remove_scroll_class = function(container, scroll_type) {
        ...
        var has_class = me.the.core.property.get(container, "ui.class.contains", scroll_type + "_scroll");
        var class_name = scroll_type + "_scroll";
        if (has_class) {
            me.the.core.property.set(container, "ui.class.remove", class_name);
            me.the.core.property.set(container.var.vertical, "ui.property.broadcast", {
                "ui.class.remove": class_name
            });
            me.the.core.property.set(container.var.footer, "ui.property.broadcast", {
                "ui.class.remove": class_name
            });
        }
    };
    me.update_scroll_class = function(container, scroll_type) {
        ...
        var has_scroll = (me.the.ui.scroll.has_scroll(content, scroll_type) || scrollbar.alwaysShow) && !scrollbar.alwaysHide;
        has_scroll = has_scroll && !me.the.widget.container.isChild(container);
        var class_name = scroll_type + "_scroll";
        if (has_scroll) {
            me.the.core.property.set(container, "ui.class.add", class_name);
            me.the.core.property.set(container.var.vertical, "ui.property.broadcast", {
    ...
    me.update = {
        set: function (object) {
            ...
            var scrollbar = container.var[scroll_type];
            var content = me.the.widget.container.content(container);
            var has_scroll = me.the.ui.scroll.has_scroll(content, scroll_type);
            if(scrollbar.has_scroll !== has_scroll) {
                ...
                me.the.core.property.set(scrollbar, "ui.style.visibility", has_scroll ? "visible" : "hidden");
                scrollbar.has_scroll = has_scroll;
            }
            var has_scroll = me.the.ui.scroll.has_scroll(content, scroll_type);
            var scroll_percent = me.the.ui.scroll.scroll_percent(content, scroll_type);
            var thumb_percent = me.the.ui.scroll.thumb_percent(content, scroll_type);
            var track_region = me.the.ui.rect.relative_region(scrollbar.var.track);
            var thumb_region = me.the.ui.rect.relative_region(scrollbar.var.thumb);
            var position = 0, size = 0;
            if (scroll_percent) {
                var length = me.the.ui.scroll.length(scroll_type, track_region, thumb_region);
                position = me.the.ui.scroll.percent_to_pos(length, scroll_percent);
            }
            if(thumb_percent) {
                var length = me.the.ui.scroll.length(scroll_type, track_region, null);
                size = me.the.ui.scroll.percent_to_pos(length, thumb_percent);
            }
            if(has_scroll) {
                me.the.ui.scroll.set_size(scrollbar.var.thumb, scroll_type, size);
            }
            var changed = me.the.ui.scroll.set_pos(scrollbar.var.thumb, scroll_type, position);
            if(changed) {
                var scrolledInfo = {};
                scrolledInfo[scroll_type] = me.the.ui.scroll.current_pos(content, scroll_type);
                me.the.core.property.set(container, "scrolled", scrolledInfo);
            }
        }
    };
    me.refresh = {
    ...
    me.before = {
        set: function (object, value) {
            ...
            me.the.ui.scroll.by(content, scroll_type, -scrollSize);
    ...
    me.scrollTo = {
        get: function (object) {
            var container = me.container(object);
            var content = me.the.widget.container.content(container);
            return me.the.ui.scroll.current_pos(content, scroll_type);
        },
        set: function (object, value) {
            var container = me.container(object);
            var content = me.the.widget.container.content(container);
            me.the.ui.scroll.set_current_pos(content, scroll_type, value);
            me.refresh.set(container);
        }
    };
    me.track = {
        set: function (object, value) {
            ...
            var container = me.container(object);
            var content = me.the.widget.container.content(container);
            var thumb_region = me.the.ui.rect.absolute_region(object.parentNode.var.thumb);
            var scroll_direction = me.the.ui.scroll.direction(value, scroll_type, thumb_region);
            var scrollbar = container.var[scroll_type];
            var scrollSize = scrollbar.scrollSize;
            if(scrollbar.snapToPage) {
                scrollSize = scrollbar.pageSize;
            ...
            if (scroll_direction < 0) {
                me.the.ui.scroll.by(content, scroll_type, 0 - scrollSize);
            }
            if (scroll_direction > 0) {
                me.the.ui.scroll.by(content, scroll_type, scrollSize);
            }
        ...
        set: function (object, value) {
            ...
            if (value) {
                object.autoScrollTimer = setInterval(function () {
                    ...
                    if (!me.the.core.property.get(window, "visible")) {
                        return;
                    }
                    var container = me.container(object);
                    var content = me.the.widget.container.content(container);
                    me.the.ui.scroll.by(content, scroll_type, scrollbar.autoScrollSize);
                    me.refresh.set(container);
    ...
    me.delta = {
        set: function (object, value) {
            ...
            scrollbar.delta_timer = setTimeout(function() {
                me.the.ui.scroll.by(content, scroll_type, scrollbar.deltaDistance);
                me.refresh.set(container);
                me.the.core.property.set(scrollbar, "snap");
    ...
    me.snap = {
        set: function(object, value) {
            ...
            scrollbar.deltaTimeout = setTimeout(function () {
                var currentPos = me.the.ui.scroll.current_pos(content, scroll_type);
                ...
                scrollbar.deltaInterval = setInterval(function () {
                    currentPos = me.the.ui.scroll.current_pos(content, scroll_type);
                    var scrollBy = 0;
                    if(direction > 0) {
                    ...
                    if(scrollBy) {
                        me.the.ui.scroll.by(content, scroll_type, scrollBy);
                        me.refresh.set(container);
                    }
                    else {
                        clearInterval(scrollbar.deltaInterval);
