COMPONENT.UIWidgetLabel = class extends COMPONENT.UIWidget {
    static config() {
        return {
            platform: "browser",
            tag: "widget-label"
        };
    }
    constructor(element) {
        super(element);
    }
    styles(element) {
        const isDefault = element.getAttribute("default");
        return {
            "background-color": isDefault ? "white" : "lightgray",
            display: "flex",
            "flex-direction": "row",
            "align-items": "center",
            "align-content": "center",
            "justify-content": "center",
            padding: "3px",
            "border-radius": "6px",
            ...this.region
        };
    }
    hover() {
        return {
            filter: "invert(10%)"
        };
    }
    touch() {
        return {
            filter: "invert(20%)"
        };
    }
    async move(pos) {
        if (pos) {
            let region = Object.assign({}, this.region);
            region.left = pos.left + "px";
            region.top = pos.top + "px";
            region.right = "";
            region.bottom = "";
            this.region = region;
            await this.updateStyles();
        }
    }
    render(element) {
        return `<widget-label-handle></widget-label-handle>
        <widget-label-label label="${element.getAttribute("label")}"></widget-label-label>`;
    }
};

COMPONENT.UIWidgetLabelHandle = class extends COMPONENT.UIWidget {
    static config() {
        return {
            platform: "browser",
            tag: "widget-label-handle"
        };
    }
    constructor(parent) {
        super(parent);
        this.move = this.attach(COMPONENT.UIWidgetLabelMove);
    }
    styles(element) {
        const isDefault = element.getRootNode().host.getAttribute("default");
        return {
            "background": "url('data:image/svg+xml;base64,') no-repeat",
            "background-size": "16px 16px",
            width: "16px",
            height: "16px",
            "border-radius": "6px",
            filter: isDefault ? "invert(25%)" : "invert(75%)"
        };
    }
};

COMPONENT.UIWidgetLabelLabel = class extends COMPONENT.UIWidget {
    static config() {
        return {
            platform: "browser",
            tag: "widget-label-label"
        };
    }
    constructor(element) {
        super(element);
        this.clipboard = this.attach(COMPONENT.UIEventClipboard);
    }
    normal() {
        return {
            "user-select": "none",
            margin: "3px"
        };
    }
    text() {
        return this.element.getAttribute("label");
    }
    render(element) {
        return `<div>${element.getAttribute("label")}</div>`;
    }
};

COMPONENT.UIWidgetLabelMove = class extends COMPONENT.UIEventMove {
    static config() {
        return {
            platform: "browser"
        };
    }
    owner() {
        return "widget-label";
    }
};